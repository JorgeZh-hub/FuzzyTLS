...
llegada = ctrl.Antecedent(np.arange(300, 1700, 1), 'llegada')
...
llegada['lenta'] = fuzz.trapmf(llegada.universe, [300, 300, 600, 700])
llegada['moderada'] = fuzz.trimf(llegada.universe, [600, 900, 1300])
llegada['rápida'] = fuzz.trapmf(llegada.universe, [900, 1300, 1700, 1700])
...
reglas = [
    ...
    ctrl.Rule(vehiculos['pocos'] & llegada['moderada'], verde['corta']),
    ctrl.Rule(vehiculos['pocos'] & llegada['rápida'], verde['larga']),
    ctrl.Rule(vehiculos['moderados'] & llegada['lenta'], verde['media']),
    ctrl.Rule(vehiculos['moderados'] & llegada['moderada'], verde['larga']),
    ctrl.Rule(vehiculos['moderados'] & llegada['rápida'], verde['larga']),
    ctrl.Rule(vehiculos['muchos'] & llegada['lenta'], verde['larga']),
...
def contar_vehiculos(lanes, tiempo_simulacion):
    ...
    with open(CSV_PATH, mode='a', newline='') as f:
        ...
        print("Cantidad vehículos: ", total)
    return total
...
def inicializar_controladores(semaforos_ids, fases_lanes_dict):
    ...
    return estado
def actualizar_controladores(estado, duracion_amarillo=3):
...
if __name__ == "__main__":
    ...
    estado = inicializar_controladores(semaforos_ids, fases_lanes_dict)
